...
//  }
  ...
  def getTempFile(filename: File): File = {
    ...
    new File(folder, folder.getName + ".csv")
  }
  ...
  def createNewLine(line: String, map: scala.collection.mutable.Map[String, Int], cities: Map[String, Int]): String = {
    ...
    s"$year$fs$make$fs$model$fs$version$fs$city$fs$month$fs$kms$fs$fair$fs$good$fs$excellent"
  }
...
//      )
//      zoomo("pricer_full_data").insert(JSON.parse(data.toString).asInstanceOf[DBObject])
//    }
